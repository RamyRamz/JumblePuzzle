...
void testJumble() {
	...
	if(trueSize != size){cerr << "Sizes do not match." <<endl;}
	if(row < 0 || row > hiddenWord.size()* 4){cerr << "Row Getter Method is invalid" << endl;}
	if(col < 0 || col > hiddenWord.size()* 3){cerr << "Column Getter Method is invalid" << endl;}
	char dir = jp->getDirection();
		if (dir != 'n' &&  dir != 'e' && dir != 's' && dir != 'w')
			cerr << "Direction does not exist" << endl;

	//Testing Puzzle
	charArrayPtr* jumble = jp->getJumble();

	if(jumble[row][col] != hiddenWord.at(0)){cerr << "Invalid Puzzle, first letter does not match Row and Column location" << endl;}
	for (int i = 1; i < hiddenWord.size(); i++){
			...
			if (jumble[row][col] != hiddenWord.at(i)){cerr <<"Invalid Puzzle Formed" << endl;}
		}//for
	...
	if(jp->getSize()==trueSize){cout<< "Size is correct" << endl;}
	else{cout << "Size is incorrect" << endl;}
	...
	try
		...
		try
		{
			jp = new JumblePuzzle("abcdefghijklmnop", "easy");
		}
		catch (BadJumbleException& e)
		{
			cerr << e.what() << endl;
		}
	/* fails taking non-strings
		try
		{
			jp = new JumblePuzzle(123456, "easy");
		}
		catch(BadJumbleException& e){
			cerr << e.what() << endl;
		}
	 */
		...
		try
		{
			jp = new JumblePuzzle(hiddenWord, "HARD"); // Must be lower-case
		}
		catch (BadJumbleException& e)
		{
			cerr << e.what() << endl;
		...
		try
		{
			jp = new JumblePuzzle(hiddenWord, "pro"); // Must be easy, medium, or hard
		}
		catch (BadJumbleException& e)
		{
			cerr << e.what() << endl;
